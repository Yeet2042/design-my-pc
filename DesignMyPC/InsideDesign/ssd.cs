...
namespace DesignMyPC.InsideDesign
{
    public partial class ssd : Form
    {
        ...
        private void LoadCard()
        {
            ...
            int endIndex = Math.Min(startIndex + CardPerPage, Global.SSD_DT.Rows.Count);
            int startOnCard = 1;
            ...
            for (int i = startIndex; i < endIndex; i++)
            {
                ...
                if (panel != null)
                {
                    ...
                    card = new ComponentCard(
                            Global.SSD_DT.Rows[i]["id"].ToString(),
                            Global.SSD_DT.Rows[i]["type"].ToString(),
                            Global.SSD_DT.Rows[i]["brand"].ToString(),
                            Global.SSD_DT.Rows[i]["serie"].ToString(),
                            Global.SSD_DT.Rows[i]["model"].ToString(),
                            ...
                            //ssd
                            Global.SSD_DT.Rows[i]["interface"].ToString(),
                            Global.SSD_DT.Rows[i]["protocal"].ToString(),
                            Global.SSD_DT.Rows[i]["size"].ToString(),
                            Global.SSD_DT.Rows[i]["read"].ToString(),
                            Global.SSD_DT.Rows[i]["write"].ToString(),
                            ...
                            Global.SSD_DT.Rows[i]["price"].ToString()
                        );
                    card.TopLevel = false;
