...
namespace DesignMyPC.InsideDesign
{
    public partial class cpu : Form
    {
        ...
        private void LoadCard()
        {
            ...
            int endIndex = Math.Min(startIndex + CardPerPage, Global.CPU_DT.Rows.Count);
            int startOnCard = 1;
            ...
            for (int i = startIndex; i < endIndex; i++)
            {
                ...
                if (panel != null)
                {
                    ...
                    card = new ComponentCard(
                            Global.CPU_DT.Rows[i]["id"].ToString(),
                            Global.CPU_DT.Rows[i]["type"].ToString(),
                            Global.CPU_DT.Rows[i]["brand"].ToString(),
                            Global.CPU_DT.Rows[i]["serie"].ToString(),
                            Global.CPU_DT.Rows[i]["model"].ToString(),

                            //cpu
                            Global.CPU_DT.Rows[i]["core"].ToString(),
                            Global.CPU_DT.Rows[i]["thread"].ToString(),
                            Global.CPU_DT.Rows[i]["bClock"].ToString(),
                            Global.CPU_DT.Rows[i]["tClock"].ToString(),
                            Global.CPU_DT.Rows[i]["tdp"].ToString(),
                            ...
                            Global.CPU_DT.Rows[i]["price"].ToString()
                        );
                    card.TopLevel = false;
